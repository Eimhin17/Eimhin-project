// Test the compatible encryption service

const crypto = require('crypto');

// Mock the compatible encryption service
...
async function testCompatibleEncryption() {
  console.log('🔍 TESTING COMPATIBLE ENCRYPTION SERVICE\n');
  
  const testMessage = "Hello, this is a test message for compatible encryption!";
  ...
  try {
    ...
    if (isMatch) {
      console.log('🎉 SUCCESS: Compatible encryption/decryption flow works correctly!');
    } else {
    ...
    console.log('');
    console.log('🛡️ Step 5: Testing tamper detection...');
    const data = JSON.parse(encrypted);
    data.ciphertext = 'tampered';
    const tampered = JSON.stringify(data);
    
    try {
      CompatibleEncryptionService.decryptMessage(tampered, user1Id, user2Id);
      console.log('❌ Tamper detection failed - should have thrown error');
    } catch (tamperError) {
      console.log('✅ Tamper detection works:', tamperError.message);
    }

  } catch (error) {
    console.error('❌ Error in compatible encryption flow:', error.message);
